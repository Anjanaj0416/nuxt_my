<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" v-if="isOpen">
    <div
      class="bg-white w-[95%] sm:w-[90%] md:w-[80%] lg:w-[60%] xl:w-[40%] max-h-screen rounded-2xl shadow-2xl flex flex-col overflow-hidden">

      <!-- Modal Header -->
      <div
        class="flex items-center justify-between px-6 py-4 text-white bg-gradient-to-r from-blue-900 to-blue-950 rounded-t-2xl">
      ...
      <div class="px-6 py-2 space-y-4 overflow-y-auto ">
          ...
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-4">
              <img
                ...
                :src="profile.image ? profile.image : 'https://img.icons8.com/ios/50/user-male-circle--v1.png'" />
              <!-- <img
                class="object-cover w-16 h-16 transition-transform rounded-full ring-2 ring-indigo-500 hover:scale-105"
            ...
            <button @click="toggleEdit1"
              class="px-4 py-2 text-xl text-blue-900 transition-colors rounded-md hover:underline hover:text-blue-900">Edit</button>
          </div>
        ...
        <div v-if="isEditing">
          <div class="flex justify-end">
            <button @click="toggleEdit1"
              class="px-4 py-2 text-xl text-blue-900 transition-colors rounded-md hover:underline hover:text-blue-900">Cancel
              Edit</button>
          </div>
      ...
      <div class="flex items-center justify-between px-6 py-4 border-t bg-gray-50">
        ...
        <button v-if="isEditing" @click="GetSave"
          class="px-4 py-2 text-white bg-indigo-700 rounded-md hover:bg-indigo-800">
          Save
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    GetSelectedImage(file) {
      console.log("GetSelectedImage:", file);
    ...
    GetSave() {
      if (this.IsValidate()) {
        this.$showConfirm("Sure to save this profile details?", "warning")
          .then(async (result) => {
