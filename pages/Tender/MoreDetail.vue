    ...
    <section class="px-2 sm:px-4 md:px-8 lg:px-24 py-6 bg-gray-50">
        <div class="grid grid-cols-1 lg:grid-cols-6 gap-6">
            <div class="lg:col-span-4 space-y-6">
              ...
              <div class="h-[90vh] overflow-y-auto space-y-6 p-2">
                <div
                  v-for="(doc, index) in tenderStore.moreDetails?.listTenderDocuments"
                  :key="index"
                  class=" p-3 "
                >
                  <a :href="imageroot + doc" target="_blank" class="block">
                    <img
                      :src="imageroot + doc"
                      class="w-full rounded-lg shadow cursor-pointer hover:opacity-80 transition"
                    />
                  </a>

                  <!-- Buttons below each image (only if more than 1 image) -->
                  <div
                    v-if="tenderStore.moreDetails?.listTenderDocuments.length > 1"
                    class="flex justify-end gap-3 mt-3"
                  >
                    <button 
                      @click="shareWhatsApp(imageroot + doc)"
                    ...
                    <button
                      @click="downloadImage(imageroot + doc)"
              ...
              <div class="flex justify-between items-center w-full mt-4"> 
                <a v-if="tenderStore.moreDetails.tenderUrl" :href="tenderStore.moreDetails.tenderUrl" target="_blank"> 
                  <button class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-xs"> 
                    Click for more information 
                  </button> 
                </a> 
                <div class="flex gap-3" v-if="tenderStore.moreDetails?.listTenderDocuments.length === 1"> 
                  <button 
                    @click="shareWhatsApp" 
                    class="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg text-xs hover:bg-green-600" 
                  > 
