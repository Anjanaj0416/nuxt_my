<template>
  <section>
    <div class="row relative">
      ...
      <div class="cssView" v-show="!ismodeedit">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-2">
          ...
          >
            ...
            <div class="break-words">{{ displayValue(field) }}</div>
          </div>
        </div>
      </div>
      <!-- End View Mode -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-2">
          ...
          >
            ...
            <div>
              ...
              <p
                v-if="err[field.name]"
                class="text-xs ml-1 text-red-800 italic animate-pulse"
              >
                {{ err[field.name] }}
              </p>
            </div>
      ...
      <!-- Controls -->
      <div class="cssfcontrol notToPrint block lg:absolute top-0 right-0 p-2 cursor-pointer">
...
export default {
  props: {
    rowData: { type: Object, required: true }, // test, patient, systemdata
    fields: { type: Array, required: true }, // [{name, label, component?, props?, format?}]
...
.cssitem > div:nth-child(2) {
  @apply text-blue-700 font-bold mt-2;
}
.cssfcontrol {
  @apply font-bold;
}
