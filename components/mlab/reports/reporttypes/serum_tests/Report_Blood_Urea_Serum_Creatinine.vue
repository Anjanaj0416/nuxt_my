  ...
  <article>
    ...
    <div class="bg-EF-green-300 p-4 my-1 rounded-lg">
      <div class="grid grid-cols-1 gap-x-2">
        <parameter_text_row :reportparameter="report_details.blood_urea" />
        <parameter_text_row :reportparameter="report_details.serum_creatinine" />
      </div>
    </div>
...
export default {
  ...
  methods: {
    getclearValues() {
      const params = ['blood_urea', 'serum_creatinine']
      params.forEach((key) => {
        if (this.report_details[key]) this.report_details[key].value = 0.0
      })
