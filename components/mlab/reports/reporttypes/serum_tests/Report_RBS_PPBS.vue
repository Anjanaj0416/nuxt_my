...
<article>
  ...
  <div class="bg-EF-green-300 p-4 my-1 rounded-lg">
    <div class="grid grid-cols-1 gap-x-2">
      <parameter_text_row :reportparameter="report_details.random_blood_sugar" />
      <parameter_text_row :reportparameter="report_details.post_prandial_blood" />
    </div>
  </div>
...
export default {
  ...
  methods: {
    getclearValues() {
      // Safely clear values
      if (this.report_details.random_blood_sugar) {
        this.report_details.random_blood_sugar.value = 0.0
      }
      if (this.report_details.post_prandial_blood) {
        this.report_details.post_prandial_blood.value = 0.0
      }
      // Optionally clear remarks
