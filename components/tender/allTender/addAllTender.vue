        ...
        <div class="modal-content">
            <div class="form-content">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <select
                        v-model="TenderTypeId"
                        class="w-full p-2 border rounded-md text-sm bg-white focus:ring-2 focus:ring-blue-400"
                        @input="clearErrorOnInput('TenderTypeId')"
                    >
                        <option disabled value="">Select Type</option>
                        <option
                        v-for="type in tenderStore.listTenderType"
                        :key="type.id"
                        :value="type.id"
                        >
                        {{ type.value }}
                    ...
                    <p v-if="err.TenderTypeId" class="mt-2 text-sm text-red-600">
                        {{ err.TenderTypeId }}
                    </p>
                    </div>
                </div>
  ...
  methods: {
      ...
      this.$showConfirm("Are you sure you want publish this tender?", "warning")
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            formData.append("TenderTypeId", this.TenderTypeId);
    ...
    IsValidate() {
      ...
      if (!this.TenderTypeId) {
        this.err.TenderTypeId = "Please select tender type";
        IsValidate = false;
      }
