<template>
  <div>
    <div 
      v-if="isOpen" 
      class="fixed inset-0 z-30 bg-white bg-opacity-50" 
      @click="$emit('close-sidebar')">
    </div>

    <aside 
      :class="[
        'fixed inset-y-0 left-0 z-40 w-64 bg-white text-gray-800 overflow-y-auto shadow-lg transform transition-transform duration-300 ease-in-out',
    ...
    >
      ...
      <div class="flex items-center justify-between px-6 py-4 bg-white shadow-md">
        ...
        <button 
          @click="$emit('close-sidebar')"
          class="p-2 transition duration-200 rounded-full focus:outline-none hover:bg-gray-100"
