    ...
    <div v-for="(item, index) in accordions" :key="index">
      ...
      >
        ...
        >
            ...
            <div 
              class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6 mt-2"
               v-for="(list, index) in welfareStore.listMemberShipPayment"
            >
                <div>
                  <p class="text-xs font-semibold text-gray-500 uppercase">Fee</p>
                  <p class="mt-1 text-sm font-sm text-gray-800">
                      {{ list.fee }}
                  </p>
                </div>
                <div>
                  <p class="text-xs font-semibold text-gray-500 uppercase">Collection Month</p>
                  <p class="mt-1 text-sm font-sm text-gray-800">
                      {{ list.collectionMonth }}
                  </p>
                </div>
                <div>
                  <p class="text-xs font-semibold text-gray-500 uppercase">Paid Date</p>
                  <p class="mt-1 text-sm font-sm text-gray-800">
                      {{ list.paidDate }}
                  </p>
                </div>
