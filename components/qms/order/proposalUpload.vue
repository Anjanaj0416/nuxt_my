      ...
      <div class="modal-content">
        <div class="form-content">
          <div class="grid grid-cols-2 gap-4 mt-1 sm:grid-cols-1 md:grid-cols-1">
            ...
            <hr />
            <div>
              <label class="block text-sm mb-2 font-bold text-gray-600">Attached the Proposal Document</label>
...
export default {
  ...
  async SetApprove() {
    ...
    formData.append("ScanedImage", this.ApprovalMemo);

    // show loading
    const loadingAlert = this.showLoading ? this.showLoading("") : null;

    try {
      const result = await this.orderStore.GetPraposalDocument(formData);
