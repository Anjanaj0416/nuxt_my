  ...
  <div>
    ...
    >
      <div class="flex items-center justify-between px-6 py-3 bg-gray-900">
        <router-link to="/"> <img  src="/assets/img/LogoDigitalTechLab.png"  alt="Logo" class="w-8 h-8 mr-2" /></router-link>
          ...
          <li v-for="link in links" :key="link.name">
            ...
            <div v-if="link.submenu">
              <div
                ...
                class="flex items-center px-4 py-3 mb-2 font-medium text-gray-300 cursor-pointer hover:bg-blue-900 hover:text-white transition duration-200 ease-in-out"
              >
                <svg
                  class="mr-4 text-gray-300 fill-current"
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <div v-show="isSubmenuOpen(link.name)" class="pl-8 space-y-2">
                <router-link
                  ...
                  :to="submenu.to"
                  @click="$emit('close-sidebar')"
                  class="flex items-center px-4 py-2 text-gray-400 hover:bg-blue-800 hover:text-white transition duration-200 ease-in-out"
                >
                  {{ submenu.name }}
                </router-link>
              </div>
    ...
    return {
      links: [
        ...
        },
        {
          ...
          icon: 'M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4s-4 1.79-4 4s1.79 4 4 4zM12 14c-4.42 0-8 2.79-8 6v2h16v-2c0-3.21-3.58-6-8-6z',
          submenu: [
            { name: 'MonEnd Order Summery', to: '/reports/MonEndOrderSummery' },
            { name: 'Payment Collection', to: '/reports/PaymentCollection' },
